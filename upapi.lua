...
function createFirebaseUser( xid )
	-- Create an entry on the firebase database for recorind purposes
	local function createFirebaseUserCallback(failed, result)
		if not failed then
			print("User added to Firebase.")
			print(result)
		else
			print("Unable to add user. Try again")
		end
	...
	local xid_json = json.encode(data)
	print(xid_json)
	rawPUTRequest(storyboard.states.firebaseURL, xid_json, createFirebaseUserCallback)
...
function getUserMetrics( metric, callback )
	...
	local additionalInfo = ""--upapi.getXID()

	rawGETRequest( metricsBaseURL .. metric, loginInfo, callback)
end
...
function rawGETRequest( url, rawdata, callback )
